...
router.get('/', async (req, res) => {
  try {
    const items = await prisma.soportes_documentales.findMany({ orderBy: { clave: 'asc' } });
    return successResponse(res, items);
  } catch (e) {
    console.error(e);
    return errorResponse(res, 'Error obteniendo soportes documentales', 500, e.message);
  }
});

router.get('/:id', async (req, res) => {
  try {
    const item = await prisma.soportes_documentales.findUnique({ where: { id: parseInt(req.params.id) } });
...
router.post('/', async (req, res) => {
  try {
    ...
    const exists = await prisma.soportes_documentales.findFirst({ where: { clave } });
    if (exists) return errorResponse(res, 'clave ya existe', 400);
    const created = await prisma.soportes_documentales.create({ data: { clave, nombre, descripcion } });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const exists = await prisma.soportes_documentales.findUnique({ where: { id } });
    if (!exists) return errorResponse(res, 'No encontrado', 404);
    const updated = await prisma.soportes_documentales.update({ where: { id }, data: req.body });
...
router.delete('/:id', async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const docsCount = await prisma.documentos.count({ where: { soporte_id: id } });
    if (docsCount > 0) return errorResponse(res, 'No se puede eliminar: tiene documentos asociados', 400);
    await prisma.soportes_documentales.delete({ where: { id } });
